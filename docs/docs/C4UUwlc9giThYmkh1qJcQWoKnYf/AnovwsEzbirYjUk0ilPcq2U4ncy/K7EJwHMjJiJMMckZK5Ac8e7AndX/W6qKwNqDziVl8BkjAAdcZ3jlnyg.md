...
2.mod对脸部的Position、Texcoord、Blend进行了修改，导致hash更新后脸部碎坏。不建议对脸部模型进行大的变动，这将导致hash更新后无法修复。正常的mod对脸部的修改仅限于一张DiffuseMap贴图。所以需要删除ini配置中关于脸部ib值的节(可以在我提供的hash表中查询)。比如查找`hash = e30ca87f`进行删除，如下
